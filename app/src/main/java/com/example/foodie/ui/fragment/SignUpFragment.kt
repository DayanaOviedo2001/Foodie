...
class SignUpFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        viewModel.userData.observe(viewLifecycleOwner) { userData ->
            when (userData[0]) {
                "" -> {
                    Snackbar.make(binding.tvSignUp, "¡Por favor, complete todos los campos!", Snackbar.LENGTH_SHORT).show()
                }

                else -> {
                    Snackbar.make(binding.tvSignUp, "¿Estás confirmando el proceso de creación de la cuenta?", Snackbar.LENGTH_SHORT)
                        .setAction("Sí") {
